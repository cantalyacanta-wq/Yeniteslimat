...
export const Navbar: React.FC = () => {
  ...
  // Dynamic Navigation Items tailored by Role (Without the Home / Ev icon)
  const navItems: NavItem[] = React.useMemo(() => {
    ...
    // Default Customer / Guest view: Clean navigation (Home is accessible by clicking the brand title)
    return [];
    ...
    <header className="sticky top-0 z-40 bg-[#021c17]/95 backdrop-blur-md border-b border-emerald-900/60 shadow-lg w-full max-w-full text-white">
      <div className="max-w-7xl mx-auto px-3 sm:px-6">
        <div className="flex items-center justify-between h-14 sm:h-16">
          ...
          {/* Logo - Restored to classic emerald bike emblem */}
          <div
            onClick={() => {
              setCurrentView('home');
            }}
          ...
          >
            ...
            <div className="truncate">
              <div className="flex items-center gap-1.5">
                <span className="font-extrabold text-xs sm:text-sm md:text-base text-white tracking-tight">
                  Antalya <span className="text-emerald-400">Şehir İçi Teslimat</span>
                </span>
                <span className="inline-block text-[10px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded bg-emerald-600 text-white shadow-xs">
                  7/24
                </span>
              </div>
              <p className="text-[10px] text-emerald-400/80 hidden sm:block truncate">Acil Moto Kurye & Hızlı Paket Havuzu</p>
            </div>
          </div>

          {/* Navigation Items (if any for active courier/admin) */}
          {navItems.length > 0 && (
              ...
              {navItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`relative flex items-center gap-1.5 sm:gap-2 px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-xl text-xs font-bold transition cursor-pointer select-none ${
                      isActive
          ...
          {/* Right Section: User Profile & Dedicated Courier Login */}
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            {currentUser.id !== 'user-guest-01' && currentUser.email ? (
              <>
                ...
                <div className="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1.5 rounded-xl border border-emerald-800/60 bg-[#03241d] text-xs text-white">
                  <div
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 bg-rose-950/60 hover:bg-rose-900 text-rose-200 border border-rose-700/60 rounded-xl text-xs font-bold transition cursor-pointer active:scale-95 shadow-xs"
                >
